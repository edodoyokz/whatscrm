...
async function processBaileysMsg({ body, uid, userFromMysql, chatId }) {
  try {
    ...
    // Process with AI if it's an incoming message
    if (!body.key?.fromMe && msgContext.type === "text") {
      try {
        const aiMessageData = {
          userId: uid,
          phone: newMessage.senderMobile,
        ...
        // Process with AI (async, don't wait for response)
        MessageProcessor.processIncomingMessage(aiMessageData)
          .then(result => {
            if (result.success) {
              console.log(`✅ AI processed message for ${newMessage.senderMobile}`);
            } else {
              console.log(`⚠️ AI processing failed for ${newMessage.senderMobile}:`, result.error);
            }
          })
          .catch(error => {
            console.error(`❌ AI processing error for ${newMessage.senderMobile}:`, error);
          });
      } catch (error) {
